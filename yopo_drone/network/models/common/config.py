"""Configuration helpers for YOPO train/test models."""
...
class YopoModelConfig:
    ...
    def dataset_root(self) -> Path:
        env_override = os.environ.get("YOPO_MODEL_DATASET_PATH")
...
class Config:
    """Dictionary-like wrapper aligned with the original YOPO code style."""

    _DERIVED_KEYS = {"goal_length", "sgm_time", "traj_num"}

    def __init__(self, values: YopoModelConfig | None = None):
        self._values = values or YopoModelConfig()
    ...
    def copy(self) -> "Config":
        data = asdict(self._values)
        return Config(YopoModelConfig(**data))

    def dataset_root(self) -> Path:
        return self._values.dataset_root()


def ensure_config(config: Config | YopoModelConfig | Mapping[str, Any] | None = None) -> Config:
    if config is None:
        return cfg
    if isinstance(config, Config):
        return config
    if isinstance(config, YopoModelConfig):
...
def make_config(
    config: Config | YopoModelConfig | Mapping[str, Any] | None = None,
    *,
